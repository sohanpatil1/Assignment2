...
class monteCarloAI(connect4Player):
	...
	def play(self, env, move):
		...
		while True:
			...
			turnout = self.playRandomGame(deepcopy(env), first_move)	#To find out who one the game
	...
	def playRandomGame(self, env, first_move):
		...
		while not env.gameOver(move, player):
			player = switch[player]	#Switch players using dictionary
			possible = env.topPosition >= 0
			indices = []
			for i, p in enumerate(possible):
				if p: indices.append(i)
	...
	def simulateMove(self, env, move, player):
		env.board[env.topPosition[move]][move] = player	#Fill in player in board[row][col]
		env.topPosition[move] -= 1
		env.history[0].append(move)	#Only in history not in deepcopy, hence used to simulate move
